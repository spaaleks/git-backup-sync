...
export async function syncWiki({ repo, mapping, destConn, project, config, srcGit, destGit, rlog }) {
  ...
  try {
    ...
    try {
      await fetchMirror(dir, sourceUrl, srcGit);
    } catch (err) {
    ...
    await pushMirror(dir, destUrl, destGit, 'refspecs');
